            ...
            VStack {
                ...
                Button {
                    if !name.isEmpty && !title.isEmpty {
                        if let image = selectedImage {
                            let award = Award(name: name, title: title, date: date, imageData: image.jpegData(compressionQuality: 1.0))
                            viewModel.addAward(award)
                        } else {
                            let award = Award(name: name, title: title, date: date)
                            viewModel.addAward(award)
                        }
                        showAddProductSheet = false
                    }
...
#Preview {
    NewAwardUIView(viewModel: SettingsViewModel(), showAddProductSheet: .constant(true))
}
